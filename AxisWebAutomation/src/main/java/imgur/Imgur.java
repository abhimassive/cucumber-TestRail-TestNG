...
public class Imgur {
	...
	public Object sendRequest(String method, String uri, Object data)
		...
		while (true) {
			try {
				...
				if (status != 200) {
					// if the access token has expired, request a new one and save to file, the
					// request will be re-sent with the new token
					if (status == 403) {
						requestAccessToken();
					} else {
						...
						if (istream == null) {
							throw new APIException(
									"API return HTTP " + status + " (No additional error message received)");
						}
					}
				} else {
				...
				if (status != 200) {
					...
					// if error from Imgur is "too many requests" wait the required amount of time
					// before re-trying
					if (status == 429) {
						JSONObject obj = (JSONObject) result;
						getWaitTime(obj);
					}
					// if we have tried to send the request the max amount of times throw an
					// exception
					if (count == maxTries || count > maxTries) {
						// Assert.fail();
						throw new APIException("API returned HTTP " + status + "(" + error + ")" + " Full response = "
								+ result.toString());
					}
	...
	public void requestAccessToken() throws MalformedURLException, IOException, APIException {
		...
		if (status != 200) {
			...
			throw new APIException(
					"API returned HTTP " + status + "(" + error + ")" + " Full response = " + result.toString());
		}
		// return resultToken;
	}
	...
	public void getWaitTime(JSONObject obj) throws InterruptedException {
		...
		System.out.println(
				"Imgur reporting too many requests. Waiting for " + minutes + " minutes; Time now: " + timeStamp);
